...
public class FrozenGame extends GameScreen {
  ...
  public FrozenGame(GameView gameView,
                    BmpWrap background_arg,
                    BmpWrap[] bubbles_arg,
                      ...
                      boolean ats_touch_fire, double ats_touch_dx)
  {
    ...
    if (endOfGame) {
      if (levelCompleted) {
        if(levelManager.getLevelIndex()+1 > Integer.parseInt(CommonFunctions.GetUserStatus("level")))
        {
          CommonFunctions.SaveUserStatusToDB("level", levelManager.getLevelIndex()+1);
          // インタースティシャル広告を出す
          if(levelManager.getLevelIndex() % 2 == 1)
          {
            myGameView.ToShowInterstitialAd();
          }
        }
      }
      ...
      if (move[FIRE] == KEY_UP && readyToFire) {
        if (levelCompleted) {
          ...
          myGameView.ToReloadInterstitialAd();
        }
        return true;
      } else {
  ...
  public void paint(Canvas c, double scale, int dx, int dy) {
    compressor.paint(c, scale, dx, dy-350);
    if (FrozenBubble.getMode() == FrozenBubble.GAME_NORMAL) {
      nextBubble.changeImage(bubbles[nextColor]);
    } else {
      nextBubble.changeImage(bubblesBlind[nextColor]);
